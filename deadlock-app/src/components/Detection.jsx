...
export default function Detection() {
  ...
  edges.forEach(req => {
     if (req.type === 'request') {
         ...
         holders.forEach(assign => {
            wfgEdges.push({ source: req.source, target: assign.target });
         });
     }
  });
  ...
  const runDetection = () => {
     ...
     dispatch({ type: 'ADD_LOG', payload: { msg: '> Initiated DFS on Wait-For Graph.', type: 'info' } });
     ...
     const dfsStep = async (nodeId, path) => {
         setActiveCodeLine(1);
         vSet.add(nodeId);
         setVisited(Array.from(vSet));
         ...
         for (let next of neighbors) {
             ...
             if (stack.includes(next)) {
                 setActiveCodeLine(6);
                 const cyclePath = stack.slice(stack.indexOf(next));
                 cyclePath.push(next);
                 playAlarm();
                 setDetectedCycle(cyclePath);
                 dispatch({ type: 'ADD_LOG', payload: { msg: `> Cycle found: ${cyclePath.join(' -> ')}`, type: 'error' } });
                 return true;
             }
             setActiveCodeLine(8);
             if (!vSet.has(next)) {
                 setActiveCodeLine(9);
                 const found = await dfsStep(next, [...path, next]);
     ...
     const runAll = async () => {
         ...
         setActiveCodeLine(-1);
         playSuccess();
         dispatch({ type: 'ADD_LOG', payload: { msg: '> DFS complete. Graph is acyclic.', type: 'info' } });
     };
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-6 pt-6 shrink-0 shadow-sm rounded-t-xl flex justify-between items-end">
         <div>
            <div className="flex items-center gap-3 mb-6">
               ...
               <div>
                  <h2 className="text-2xl font-bold tracking-tight text-slate-100">Deadlock Detection</h2>
                  <p className="text-sm font-medium text-slate-400">Wait-For Graph & Cycle Detection</p>
               </div>
            </div>
            ...
            <div className="flex gap-4">
               {tabs.map(tab => (
                  ...
                  >
                     {tab.icon} {tab.label}
                     {activeTab === tab.id && (
                        <motion.div layoutId="detectTab" className="absolute bottom-0 left-0 right-0 h-1 bg-blue-500 rounded-t" />
                     )}
                  </button>
               ))}
            </div>
         ...
         <div className="pb-3 flex gap-2">
            <button onClick={runDetection} disabled={isRunning || processNodes.length === 0} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-2 px-6 rounded-lg text-sm flex items-center gap-2 shadow-sm transition-colors">
               <Play size={16}/> Execute DFS
            </button>
         </div>
      </div>
            ...
            {activeTab === 'wfg' && (
               <motion.div key="wfg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="p-8 h-full overflow-y-auto custom-scrollbar flex flex-col items-center bg-slate-950">
                  <div className="max-w-2xl w-full">
                     <p className="text-slate-300 bg-slate-900 p-6 border border-slate-800 rounded-xl shadow-sm mb-8 leading-relaxed text-center">
                        The <strong>Wait-For Graph (WFG)</strong> is a collapsed version of the Resource Allocation Graph. 
                        It directly maps Process → Process requests by eliminating single-instance resource nodes. A cycle in the WFG guarantees a cycle in the RAG.
                     </p>
                     
                     <div className="space-y-4">
                        {processNodes.length === 0 ? (
                           <p className="text-slate-500 text-center py-10 border-2 border-dashed border-slate-800 rounded-xl">No processes in system.</p>
                        ) : wfgEdges.length === 0 ? (
                           <p className="text-slate-500 text-center py-10 border-2 border-dashed border-slate-800 rounded-xl bg-slate-900/50">No active Wait-For dependencies.</p>
                        ) : (
                           wfgEdges.map((edge, idx) => {
                              ...
                              return (
                                 <div key={idx} className={`p-5 rounded-xl border flex items-center justify-center gap-8 transition-all duration-300 shadow-sm ${
                                    isCycle ? 'bg-red-900/30 border-red-800/50 text-red-300' :
                                    isActive ? 'bg-blue-900/30 border-blue-800/50 text-blue-300' :
                                    'bg-slate-900 border-slate-800 text-slate-300'
                                 }`}>
                                    <div className={`w-14 h-14 rounded-lg flex items-center justify-center font-bold text-xl border ${isActive || isCycle ? 'bg-slate-800 border-blue-500/50' : 'bg-slate-950 border-slate-800'}`}>
                                       {edge.source}
                                    </div>
                                    <div className="w-32 h-[2px] bg-slate-800 relative">
                                       ...
                                       {(isActive || isCycle) && (
                                          <motion.div 
                                             className={`absolute top-0 bottom-0 left-0 ${isCycle ? 'bg-red-500' : 'bg-blue-500'}`}
            ...
            {activeTab === 'trace' && (
               <motion.div key="trace" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="h-full flex flex-col md:flex-row bg-slate-950">
                  ...
                  <div className="flex-1 border-r border-slate-800 bg-slate-900 relative p-8 overflow-hidden flex flex-col">
                     <h3 className="text-sm font-bold text-slate-500 mb-6 flex items-center gap-2 shrink-0 uppercase tracking-wider">
                        <Code size={16}/> DFS Algorithm Source
                     </h3>
                     <div className="font-mono text-sm leading-loose relative flex-1 overflow-y-auto custom-scrollbar bg-slate-950 p-6 rounded-xl border border-slate-800 shadow-inner">
                        {DFS_CODE.map((line, idx) => (
                           <div key={idx} className={`relative z-10 px-2 transition-colors duration-200 ${activeCodeLine === idx ? 'bg-blue-900/30 text-blue-300 font-bold border-l-4 border-blue-500' : 'text-slate-400 border-l-4 border-transparent'}`}>
                              ...
                              <span dangerouslySetInnerHTML={{__html: line.replace(/let|function|return|while|if|break|true|false/g, match => `<span class="text-blue-400">${match}</span>`).replace(/\/\/.*$/g, match => `<span class="text-emerald-500 italic">${match}</span>`)}} />
                           </div>
                        ))}
                     </div>
                  </div>
                  ...
                  <div className="w-80 bg-slate-950 p-8 flex flex-col border-l border-slate-800">
                     <h3 className="text-sm font-bold text-slate-500 mb-6 flex items-center gap-2 shrink-0 uppercase tracking-wider">
                        <Database size={16}/> Recursion Stack
                     </h3>
                     <div className="flex-1 flex flex-col justify-end gap-3 overflow-y-auto custom-scrollbar bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-sm">
                        {dfsStack.length === 0 ? (
                           <div className="text-slate-600 text-sm font-medium text-center pb-10">Stack is empty.</div>
                        ) : (
                           dfsStack.map((node, i) => (
                              <motion.div 
                                 key={`${node}-${i}`}
                                 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                 className={`px-4 py-3 border text-center font-bold text-lg rounded-lg shadow-sm ${
                                    i === dfsStack.length - 1 ? 'bg-blue-900/50 border-blue-500/50 text-blue-400 scale-105' : 'bg-slate-800 border-slate-700 text-slate-300'
                                 }`}
                              >
                                 {node}
                              </motion.div>
                     ...
                     <AnimatePresence>
                       {detectedCycle && (
                          <motion.div 
                             ...
                             className="mt-6 p-4 bg-red-900/20 border border-red-800/50 rounded-xl text-red-400 shadow-sm"
                          >
